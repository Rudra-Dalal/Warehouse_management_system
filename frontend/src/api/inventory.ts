...
export async function getInventoryApi(params?: GetInventoryParams): Promise<Inventory[]> {
  ...
  // Support both /v1/inventory? and legacy /v1/inventory/? if warehouse_id passed in unit tests
  const base = params?.warehouse_id ? "/v1/inventory/" : "/v1/inventory";
  const endpoint = `${base}${queryStr ? `?${queryStr}` : ""}`;
